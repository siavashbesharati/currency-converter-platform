...
- [ ] **Configure JWT auth and RBAC:** implement `AddAuth` to configure `JwtBearer` using `JwtSettings` from `appsettings.json`; add role-based policy and protect endpoints.
- [ ] **Register HttpClient + Polly:** configure named `HttpClient` for Frankfurter with retry (exponential backoff + jitter) and circuit-breaker policies.
